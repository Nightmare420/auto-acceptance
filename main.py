...
def _iter_attrs(attrs_obj):
    if isinstance(attrs_obj, dict):
        return list(attrs_obj.values())
    if isinstance(attrs_obj, list):
        return attrs_obj
    return []
...
async def get_or_create_manufacturer_attr_meta(client: httpx.AsyncClient, auto_create: bool = True) -> Dict[str, Any]:
    ...
    r = await client.post(
        f"{MS_API}/entity/product/metadata/attributes",
...
async def import_invoice_preview(
    ...
    parsed = parse_invoice_like_excel(file.file)
    if parsed.empty:
